package com.novoda.fenster.video;
...
public class TextureVideoView extends android.view.TextureView implements Player {
    ...
    private VideoController mConcertPlayerController;
    ...
    public void setVideo(final String path, final int seekInSeconds) {
        setVideo(Uri.parse(path), seekInSeconds);
    }
    ...
    public void setVideoURI(final Uri uri, final Map<String, String> headers, final int seekInSeconds) {
//        Log.i("start playing: " + uri);
        mUri = uri;
        mHeaders = headers;
    ...
    public void enablePieceNavigators(final boolean isFirstPiece, final boolean isLastPiece) {
        if (isFirstPiece) {
            mConcertPlayerController.hidePreviousPieceButton();
        }
        if (isLastPiece) {
            mConcertPlayerController.hideNextPieceButton();
        }
    }
    ...
    private MediaPlayer.OnPreparedListener mPreparedListener = new MediaPlayer.OnPreparedListener() {
        ...
        public void onPrepared(final MediaPlayer mp) {
            ...
            if (videoSizeCalculator.hasASizeYet()) {
                ...
                // so we won't get a "surface changed" callback, so start the video here instead of in the callback.
...
//            Log.d("Error: " + frameworkError + "," + implError);
...
//            Log.e("VideoView error. File or network related operation errors.");
...
//                Log.d("Dismissing last error dialog for a new one");
                errorDialog.dismiss();
            }
...
//            Log.e("VideoView error. Bitstream is not conforming to the related coding standard or file spec.");
//        } else if (frameworkError == MediaPlayer.MEDIA_ERROR_SERVER_DIED) {
//            Log.e("VideoView error. Media server died. In this case, the application must release the MediaPlayer object and instantiate a new one.");
//        } else if (frameworkError == MediaPlayer.MEDIA_ERROR_TIMED_OUT) {
//            Log.e("VideoView error. Some operation takes too long to complete, usually more than 3-5 seconds.");
//        } else if (frameworkError == MediaPlayer.MEDIA_ERROR_UNKNOWN) {
//            Log.e("VideoView error. Unspecified media player error.");
//        } else if (frameworkError == MediaPlayer.MEDIA_ERROR_UNSUPPORTED) {
//            Log.e("VideoView error. Bitstream is conforming to the related coding standard or file spec, but the media framework does not support the feature.");
//        } else if (frameworkError == MediaPlayer.MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK) {
//            Log.e("VideoView error. The video is streamed and its container is not valid for progressive playback i.e the video's index (e.g moov atom) is not at the start of the file.");
